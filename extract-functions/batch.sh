...
echo "Extracting Functions for ${SUBMISSION_ID}"
...
# EXTRAT FUNCTIONS CALL FOR EACH USER IN THE SUBMISSION
find "./data/${HACK_ID}/submissions/${SUBMISSION_ID}/" \
    ...
    | while read USER_ID; do
        ...
        ./extract-functions/extract_functions.py --hack ${HACK_ID} --submission ${SUBMISSION_ID} --user ${USER_ID}
    done
